...
from approaches.directchat import DirectChatApproach
...
async def setup_clients():
    ...
    azure_credential: AzureCliCredential | AzureDeveloperCliCredential | ManagedIdentityCredential
    azure_ai_token_provider: Callable[[], Awaitable[str]]
    if os.getenv("USE_AZ_CLI_CREDENTIAL"):
        current_app.logger.info("Setting up Azure credential using AzureCliCredential (az login)")
        azure_credential = AzureCliCredential(tenant_id=AZURE_TENANT_ID) if AZURE_TENANT_ID else AzureCliCredential()
    elif RUNNING_ON_AZURE:
    ...
    search_client = (
        SearchClient(endpoint=AZURE_SEARCH_ENDPOINT, index_name=AZURE_SEARCH_INDEX, credential=azure_credential)
    ...
    # Use DirectChatApproach (no RAG) when Azure AI Search is not configured
    if AZURE_SEARCH_SERVICE:
        ...
        current_app.logger.info("AZURE_SEARCH_SERVICE not set — using DirectChatApproach (no RAG)")
        current_app.config[CONFIG_CHAT_APPROACH] = DirectChatApproach(
            openai_client=openai_client,
            chatgpt_model=OPENAI_CHATGPT_MODEL,
            chatgpt_deployment=AZURE_OPENAI_CHATGPT_DEPLOYMENT,
            prompt_manager=prompt_manager,
...
async def close_clients():
    if sc := current_app.config.get(CONFIG_SEARCH_CLIENT):
        await sc.close()
