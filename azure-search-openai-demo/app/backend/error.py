...
# Imported lazily to avoid circular imports — resolved at call time
def _airs_blocked_error_class():
    from approaches.directchat import AIRSBlockedError
    return AIRSBlockedError
...
def is_airs_blocked_error(error: Exception) -> bool:
    return isinstance(error, _airs_blocked_error_class())


def error_dict(error: Exception) -> dict:
    if is_airs_blocked_error(error):
        return {"error": f"🛡️ PRISMA AIRS SECURITY ALERT — This request was blocked by Prisma AI Runtime Security.\nThreat category: {error.category}"}  # type: ignore[attr-defined]
